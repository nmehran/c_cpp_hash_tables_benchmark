/**
 * @file definitions/shims/folly_f14_fast/shim.h
 * @brief This shim adapts `folly::F14FastMap` to the benchmark suite's
...
/**
 * @brief Adapter that conforms `folly::F14FastMap` to the benchmark API.
 * @tparam blueprint A type providing key_type, value_type, hash_key, and cmpr_keys.
 */
template< typename blueprint > struct folly_f14_fast
{
  ...
  using table_type = folly::F14FastMap<
  ...
  {
    table_type table;
    // F14FastMap's default max_load_factor is 0.5.
    // The actual max_load_factor will be min(MAX_LOAD_FACTOR, Policy::kMaxLoadFactor).
    // Policy::kMaxLoadFactor is 1.0 for NodePolicy and 0.8 for Value/Vector Policies.
 ...
 */
template<> struct folly_f14_fast< void >
{
  /// The official name for display in plots and reports.
  static constexpr const char *label = "folly_f14_fast";
  /// The color used for this table in generated plots (RGB).
  static constexpr const char *color = "rgb( 135, 206, 235 )"; // SkyBlue
