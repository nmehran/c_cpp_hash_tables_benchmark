/**
 * @file definitions/shims/folly_f14_vector/shim.h
 * @brief This shim adapts `folly::F14VectorMap` to the benchmark suite's
...
// Emit a compile-time advisory that this table's effective max_load_factor is capped.
#if !defined(FOLLY_F14_VECTOR_LOAD_FACTOR_ADVISORY_EMITTED)
    #define FOLLY_F14_VECTOR_LOAD_FACTOR_ADVISORY_EMITTED
    #define FOLLY_F14_VECTOR_ADVISORY_MSG \
        "folly_f14_vector Shim Advisory: This table's effective max_load_factor is capped at 0.8. The benchmark's global MAX_LOAD_FACTOR may not be fully applied if higher."
    #if defined(_MSC_VER)
        #pragma message(FOLLY_F14_VECTOR_ADVISORY_MSG)
 ...
 */
template< typename blueprint > struct folly_f14_vector
{
  ...
  using table_type = folly::F14VectorMap<
    typename blueprint::key_type,
    typename blueprint::value_type, // This is the Mapped type for F14Map
    hash,
    cmpr
  ...
  {
    table_type table;
    // F14VectorMap's Policy::kMaxLoadFactor is 0.8.
    // The actual max_load_factor will be min(MAX_LOAD_FACTOR, Policy::kMaxLoadFactor).
    // This means F14VectorMap will respect the benchmark's global MAX_LOAD_FACTOR
    // only if it is less than or equal to 0.8.
 ...
 */
template<> struct folly_f14_vector< void >
{
  /// The official name for display in plots and reports.
  static constexpr const char *label = "folly_f14_vector";
  /// The color used for this table in generated plots (RGB).
  static constexpr const char *color = "rgb( 65, 105, 225 )"; // RoyalBlue
