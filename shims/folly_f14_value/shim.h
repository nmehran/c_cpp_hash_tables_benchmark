...
#if !defined(FOLLY_F14_VALUE_LOAD_FACTOR_ADVISORY_EMITTED)
    #define FOLLY_F14_VALUE_LOAD_FACTOR_ADVISORY_EMITTED
    #define FOLLY_F14_VALUE_ADVISORY_MSG \
        "folly_f14_value Shim Advisory: This table's effective max_load_factor is capped at 0.8. The benchmark's global MAX_LOAD_FACTOR may not be fully applied if higher."
    #if defined(_MSC_VER)
        #pragma message(FOLLY_F14_VALUE_ADVISORY_MSG)
    #elif defined(__GNUC__) || defined(__clang__)
        #pragma message FOLLY_F14_VALUE_ADVISORY_MSG
    #endif
#endif // FOLLY_F14_VALUE_LOAD_FACTOR_ADVISORY_EMITTED

#include <folly/container/F14Map.h>
#include <type_traits> // Required for std::is_nothrow_invocable_v, std::true_type

/**
 * @brief Adapter that conforms `folly::F14ValueMap` to the benchmark API.
 * @tparam blueprint A type providing key_type, value_type, hash_key, and cmpr_keys.
 */
template< typename blueprint > struct folly_f14_value
{
  ...
  {
    table_type table;
    // F14ValueMap's Policy::kMaxLoadFactor is 0.8.
    // The actual max_load_factor will be min(MAX_LOAD_FACTOR, Policy::kMaxLoadFactor).
    // This means F14ValueMap will respect the benchmark's global MAX_LOAD_FACTOR
 ...
 */
template<> struct folly_f14_value< void >
{
  /// The official name for display in plots and reports.
  static constexpr const char *label = "folly_f14_value";
  /// The color used for this table in generated plots (RGB).
  static constexpr const char *color = "rgb( 30, 144, 255 )"; // DodgerBlue
